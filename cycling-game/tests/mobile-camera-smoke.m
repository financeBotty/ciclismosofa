...
- (void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation {
    NSString *test =
        @"(() => {"
         ...
         "const teamOrderTrigger = document.getElementById('teamOrderButton');"
         "const teamOrderTriggerBox = teamOrderTrigger.getBoundingClientRect();"
         "const teamOrderHit = document.elementFromPoint(teamOrderTriggerBox.left + teamOrderTriggerBox.width / 2, teamOrderTriggerBox.top + teamOrderTriggerBox.height / 2);"
         "teamOrderHit.click();"
         ...
         "const teamOrdersValid = !teamOrderPanel.classList.contains('is-hidden') &&"
           ...
           "teamOrderHit.closest('#teamOrderButton') === teamOrderTrigger &&"
         ...
         "const pauseRect = document.getElementById('pauseButton').getBoundingClientRect();"
         "const cameraRect = switcher.getBoundingClientRect();"
         "const systemControls = document.querySelector('.race-system-controls');"
         "const systemRect = systemControls.getBoundingClientRect();"
         "const speedHit = document.elementFromPoint(speedRect.left + speedRect.width / 2, speedRect.top + speedRect.height / 2);"
         "const pauseHit = document.elementFromPoint(pauseRect.left + pauseRect.width / 2, pauseRect.top + pauseRect.height / 2);"
         "const systemControlsIntegrated = Boolean(systemRect.width > 0 && systemRect.height > 0 &&"
           "speedRect.left >= systemRect.left && speedRect.right <= systemRect.right &&"
           "cameraRect.left >= systemRect.left && cameraRect.right <= systemRect.right &&"
           "pauseRect.left >= systemRect.left && pauseRect.right <= systemRect.right &&"
           "speedRect.right <= cameraRect.left && cameraRect.right <= pauseRect.left && speedHit.closest('#raceSpeedButton') &&"
           "pauseHit.closest('#pauseButton'));"
         "const floatingNavigationSeparated = tabsRect.right <= systemRect.left + 1 &&"
           "tabsRect.bottom <= systemRect.bottom && systemControlsIntegrated;"
         ...
         "return {"
           ...
           "tabAuditValid, teamOrdersValid, teamOrderHitId: teamOrderHit.id || teamOrderHit.parentElement?.id || '', teamOrderColumns, teamOrderFontSize, teamOrderMinButtonHeight, teamOrderPanelWidth: teamOrderPanelBox.width, pauseOverlayValid, teamsDirectoryValid, teamSelectionValid, selectionButtonSticky, tutorialValid,"
           ...
           "touchTargetsValid, legacyProfileRemoved, systemControlsIntegrated,"
